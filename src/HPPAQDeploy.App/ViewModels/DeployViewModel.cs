...
public partial class DeployViewModel : ObservableObject
{
    ...
    private async Task DeployDevicesAsync(List<Device> toDeploy)
    {
        ...
        var networkCred = _credentialStore.Decrypt(SelectedCredential);
        ...
        try
        {
            ...
            SnackbarService.Show($"Deployment complete — {SuccessCount} succeeded, {FailCount} failed");
    ...
    private async Task ScanGroupAsync()
    {
        if (SelectedGroup is null)
        {
            ...
            SnackbarService.Show("Select a group before scanning");
            return;
        }
        ...
        if (GroupDevices.Count == 0)
        {
            ...
            SnackbarService.Show("No devices in this group");
            return;
        }
        ...
        if (SelectedCredential is null)
        {
            ...
            SnackbarService.Show("Select credentials first");
            return;
        }
        ...
        catch (Exception ex)
        {
            ...
            SnackbarService.Show($"Scan failed: {ex.Message}");
    ...
    private async Task CleanupRemoteFilesAsync()
    {
        ...
        foreach (var device in GroupDevices)
        {
            try
            {
                await _hpiaManager.CleanupRemoteAsync(
                    device.Hostname ?? device.IpAddress,
                    networkCred, CancellationToken.None);
                cleaned++;
            }
            catch { failed++; }
        }
        ...
        SnackbarService.Show($"Cleanup: {cleaned} devices cleaned");
    }
    ...
    private async Task ExportResultsAsync()
    {
        ...
        try
        {
            ...
            SnackbarService.Show("Deployment log exported");
    ...
    private async Task ExportScanLogAsync()
    {
        ...
        try
        {
            ...
            SnackbarService.Show("Scan log exported");
        }
