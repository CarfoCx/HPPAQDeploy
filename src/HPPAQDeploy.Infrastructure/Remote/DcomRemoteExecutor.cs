...
public class DcomRemoteExecutor : IRemoteExecutor
{
        ...
        IProgress<string>? progress = null)
    {
        ...
        catch { /* Will poll by exit code file instead */ }
        ...
        try { if (File.Exists(uncExitCodePath)) File.Delete(uncExitCodePath); } catch { }
        ...
        while (processRunning)
        {
            ...
            if (DateTime.UtcNow - startTime > timeout)
            {
                ...
                if (pid > 0)
                    try { await Task.Run(() => TryKillProcess(scope, pid)).ConfigureAwait(false); } catch { }
                // Kill any HPImageAssistant.exe still running
                try { await Task.Run(() => TryKillProcessByName(scope, "HPImageAssistant")).ConfigureAwait(false); } catch { }
            ...
            if (DateTime.UtcNow - lastProgressTime > TimeSpan.FromSeconds(10))
            {
                ...
                var msg = !string.IsNullOrEmpty(progressDetail)
                    ? $"{hostname}: {progressDetail}"
                    : $"{hostname}: Waiting for HPIA... ({elapsed.TotalSeconds:F0}s)";

                // Only report if the message actually changed
        ...
        ref int lastCount, ref string lastName)
    {
        ...
        {
            // Non-critical — progress monitoring is best-effort
        }
